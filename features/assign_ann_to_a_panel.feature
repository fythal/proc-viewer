Feature: 警報を警報パネルに割り当てる
  ...
  Scenario: 警報パネルに割り当てられている警報をパネルの他の場所に移動する
    Given 警報パネル "n1" の "a1" の場所に警報が割り当てられている
    When その警報の編集画面を表示する
    And 警報パネルについて、パネル番号に "n1"、警報の場所に "a2" を入力する
    And 手順書の編集ボタンをクリックする
    Then 警報の詳細ページが表示される
    And 正常に警報が編集されたされたメッセージが表示される
    And 警報パネルの番号は "n1"、警報の場所は "a2" となっている
